...
class BlackjackAnalyzer:
    ...
    def get_recommendation(self, state: GameState) -> Recommendation:
        ...
        if dev_key in ILLUSTRIOUS_18:
            ...
            if tc >= deviation["tc"]:
                return Recommendation(action=deviation["action"], win_probability=0.0, expected_value=0.0)
        ...
        return Recommendation(action=action, win_probability=0.0, expected_value=0.0)
